...
const Upage = () => {
    ...
    const isOwner = user?.accountType === "faculty"
    ...
    useEffect(() => {
        fetch("http://localhost:8800/unipages")
            .then(res => res.json())
    ...
    useEffect(() => {
        ...
        fetch(`http://localhost:8800/unipage/${user.userID}`)
            .then(res => res.json())
            .then(data => {
                setPage(data)
                if (data) { setUniName(data.UniName || ""); setUniDesc(data.UniDesc || "") }
            })
            .catch(err => console.error(err))
    }, [isOwner, user?.userID])
    ...
    const handleSave = async () => {
        setMessage("")
        const res = await fetch(`http://localhost:8800/unipage/${user.userID}`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ UniName, UniDesc }),
        })
        const result = await res.json()
        if (res.ok) {
            setPage({ userID: user.userID, UniName, UniDesc, approved: 0 })
            setEditing(false)
    ...
    if (selected) {
        return (
            <div style={{ padding: "40px" }}>
                ...
                <h1>{selected.UniName}</h1>
                <p>{selected.UniDesc || "No description."}</p>
            </div>
        )
    }
        ...
        <div style={{ padding: "40px" }}>
            ...
            {(!isOwner || view === "browse") && (
                <>
                    ...
                    {allPages.length === 0 ? <p>No approved university pages yet.</p> : (
                        <div>
                            {allPages.map(p => (
                                <div key={p.userID} onClick={() => setSelected(p)}
                                    ...
                                    <strong>{p.UniName}</strong>
                                    <p style={{ margin: "4px 0 0" }}>{p.UniDesc}</p>
                                </div>
                            ))}
                        </div>
                    )}
            ...
            {isOwner && view === "mine" && (
                <>
                    <h2>Your University Page</h2>
                    {page && !page.approved && <p style={{ color: "orange" }}>Pending admin approval.</p>}
                    {!editing ? (
                        <>
                            <h3>{page?.UniName || "No name set"}</h3>
                            <p>{page?.UniDesc || "No description set."}</p>
                        ...
                        <>
                            ...
                            <textarea value={UniDesc} onChange={e => setUniDesc(e.target.value)} placeholder="University Description"
